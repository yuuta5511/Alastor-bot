...
await doc.loadInfo();
const sheet = doc.sheetsByIndex[1]; // الصفحة التانية

// ====== FUNCTION TO CHECK NUMBERS ======
async function checkSheetAndSendMessages() {
  const rows = await sheet.getRows();
  for (const row of rows) {
    const channelName = row._rawData[0]; // العمود اللي فيه اسم الروم
    const number = Number(row._rawData[5]); // العمود اللي فيه الرقم
    ...
    if (number === 5) {
      ...
      await channel.send(
        `<@${user1}> <@${user2}> <@${user3}> Faster or i will call my suber visor on u ￣へ￣ `
      );
    }
