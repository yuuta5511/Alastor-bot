...
import { sheets, drive, spreadsheetId, progressSheetName, configSheetName } from './slashCommandsBot.js';
...
async function findMatchingChannel(roleName, client) {
    try {
...
async function findSubfolderByPatterns(parentFolderId, patterns) {
    try {
...
async function giveDriveAccessByRole(mainFolderId, userEmail, roleType) {
    try {
        ...
        const subfolderId = await findSubfolderByPatterns(mainFolderId, subfolderPatterns);
...
export const assignCommand = {
    ...
    async execute(interaction) {
        ...
        try {
            ...
            const targetChannel = await findMatchingChannel(projectRole.name, interaction.client);
            if (targetChannel) {
...
export async function handleAcceptButton(interaction) {
    ...
    try {
        ...
        const targetChannel = await findMatchingChannel(role.name, interaction.client);
        if (targetChannel) {
            await targetChannel.send({
